...
def load_csm_1b_local(model_path: str, device: str = "cuda", audio_num_codebooks: int = 32):
    ...
    # Check if model_path is a HuggingFace repo name or local path
    is_hf_repo = '/' in model_path and not os.path.exists(model_path)
    
    if is_hf_repo:
        print(f"Detected HuggingFace repo: {model_path}")
        from huggingface_hub import hf_hub_download
        
        # Download config.json to get model configuration
        config_path = hf_hub_download(model_path, 'config.json')
        with open(config_path) as f:
            config_dict = json.load(f)
        
        config = ModelArgs(
            backbone_flavor=config_dict.get('backbone_flavor', 'llama-1B'),
            decoder_flavor=config_dict.get('decoder_flavor', 'llama-100M'),
            text_vocab_size=config_dict.get('text_vocab_size', 128256),
            audio_vocab_size=config_dict.get('audio_vocab_size', 2051),
            audio_num_codebooks=audio_num_codebooks,
        )
    else:
        # Local path - use defaults
        config = ModelArgs(
    ...
    if is_hf_repo:
        ...
        # Local path - try to load safetensors or pytorch weights
        from safetensors.torch import load_file
        ...
        if os.path.exists(weights_path):
            ...
            # Fall back to from_pretrained for local paths that might have different structure
